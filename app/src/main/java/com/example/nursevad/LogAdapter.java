...
public class LogAdapter extends ListAdapter<LogEvent, LogAdapter.ViewHolder> {
    ...
    public LogAdapter(OnPlayClickListener listener) {
        super(new DiffUtil.ItemCallback<LogEvent>() {
            @Override public boolean areItemsTheSame(@NonNull LogEvent oldItem, @NonNull LogEvent newItem) { return oldItem.id.equals(newItem.id); }
            @Override public boolean areContentsTheSame(@NonNull LogEvent oldItem, @NonNull LogEvent newItem) { 
                return oldItem.id.equals(newItem.id) && oldItem.type == newItem.type && oldItem.level == newItem.level && oldItem.isPoni == newItem.isPoni && oldItem.isDownloadError == newItem.isDownloadError && (oldItem.displayName != null ? oldItem.displayName.equals(newItem.displayName) : newItem.displayName == null) && (oldItem.recordedSpeechUri != null ? oldItem.recordedSpeechUri.equals(newItem.recordedSpeechUri) : newItem.recordedSpeechUri == null); 
            }
        });
    ...
    class ViewHolder extends RecyclerView.ViewHolder {
        ...
        void bind(LogEvent event) {
            ...
            if (event.type == LogEvent.Type.SPEECH) {
                ...
                if (event.displayName != null && !event.displayName.isEmpty()) { tvPlayed.setText(event.displayName); tvPlayed.setVisibility(View.VISIBLE); }
                ...
                if (event.isDownloadError) {
                    ...
                    tvTitle.setText(time + " - Voice Message Error");
